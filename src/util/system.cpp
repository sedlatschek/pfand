...
const char * const BITCOIN_CONF_FILENAME = "pfandcoin.conf";
...
#else
    const char* pszModule = "pfandcoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Pfandcoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Pfandcoin";
#else
    // Unix
    return pathRet / ".pfandcoin";
#endif
#endif
}
